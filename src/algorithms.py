...
class Algorithm(object):
    def __init__(self, environment, agent, replay_buffer, training_steps=0,
            ...
            evaluation_exploration_ratio=1, return_viewer=False):
        ...
        self.evaluation_exploration_ratio = evaluation_exploration_ratio
...
class OffPolicyAlgorithm(Algorithm):
    def __init__(self, environment, agent, replay_buffer, training_steps=0,
        ...
        super(OffPolicyAlgorithm, self).__init__(
            ...
            restore_from_checkpoint, evaluation_exploration_ratio, return_viewer
        )
        ...
        self._hparams = {
            ...
            "evaluation_exploration_ratio": self.evaluation_exploration_ratio
        }
    ...
    def main_loop(self, training_steps, evaluate_every):
        ...
        while training_steps > n_done:
            if evaluation_exploration_ratio < self.evaluation_exploration_ratio:
                n_evaluations += self.collect_training_data(explore=False)
            else:
                n_explorations += self.collect_training_data(explore=True)
            evaluation_exploration_ratio = n_evaluations / n_explorations
            n_iterations_collected = n_evaluations + n_explorations
            while n_done == 0 \
                    or n_iterations_collected / n_done > self.train_every:
                train_actor=False if self.make_critic_checkpoint else True
                self.train(train_actor=train_actor)
                n_done += 1
                if n_done % 100 == 0:
    ...
    def collect_training_data(self, explore=True):
        # resent env
        print("  collecting, explore={}".format(explore), end='\r')
        max_steps = self.env.spec.max_episode_steps
        shape = (max_steps, ) + self.agent.policy_input_shape
        policy_inputs_b = np.zeros(shape=shape, dtype=self.env.observation_space.dtype)
        ...
        gym_state = self.env.reset()
        n_transitions = 0
        while True:
            states = gym_state[np.newaxis].astype(np.float32)
            ...
            estimated_return = self.agent.get_estimated_returns(
                policy_inputs,
                actions,  # self.agent.get_actions(states, explore=False) 1-step
                mode=1
            ).numpy()[0]
            gym_state, reward, done, _ = self.env.step(action.numpy())
            # store state action reward done in temp buffer
            policy_inputs_b[n_transitions] = policy_input
